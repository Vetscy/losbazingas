...
import { criarPeer, iceServers, MORTO, PRAZO_CONEXAO_MS, suportaWebRTC } from '../../shared/rtc.js';
...
function watchSlot(slot) {
  ...
  if (!info) return;
  watching.add(slot);
  if (!streams.has(slot)) openStream(slot, info.userId);
  ws?.send(JSON.stringify({ type: 'watch', slot }));
  tentarWebRtc(slot);
  // O relay continua sendo a fonte principal; se a configuração já chegou,
  // o decoder pode começar imediatamente.
  if (info.config) startStream(slot, info.config);
  renderGrid();
}
...
function renderGrid() {
  ...
  if (!inRoom()) {
    ...
    $('app').classList.remove('cheia', 'flutua', 'palco', 'conectando');
    return;
  }
  ...
  $('app').classList.toggle('conectando', !hasPeople);
  $('bottombar').hidden = !hasPeople;
...
function buildTile(p, { palco = false, semVideo = false, slot: slotDado = null } = {}) {
  ...
  if (stream) {
    tile.append(stream.rtcConnected && stream.video ? stream.video : stream.canvas);
    tile.title = palco
      ? telaCheia
    ...
    if (!stream.started) tile.append(buildLoading(stream.rtcError));
...
function buildLoading(error = '') {
  const wrap = document.createElement('div');
  wrap.className = 'tile-loading';
  wrap.innerHTML = error ? '' : '<span class="spinner"></span>';
  wrap.append(document.createTextNode(error || 'Conectando…'));
  return wrap;
}
...
/**
 * Tenta receber a mídia diretamente do transmissor. O player relay permanece
 * ativo durante a tentativa para que uma falha de ICE nunca vire tela preta.
 */
function tentarWebRtc(slot) {
  if (!suportaWebRTC() || !roomTokens || !watching.has(slot)) return;
  const s = streams.get(slot);
  if (!s || s.rtc) return;
  ...
  s.rtc = tentativa;
  tentativa.timer = setTimeout(() => fallbackRtc(slot), PRAZO_CONEXAO_MS);

  const enviar = (signal) => {
    if (!tentativa.peerId || ws?.readyState !== WebSocket.OPEN) {
      tentativa.candidatos.push(signal);
      return;
    }
    ws.send(JSON.stringify({ type: 'rtc-signal', slot, peerId: tentativa.peerId, signal }));
  };

  const receberFaixa = (event) => {
    if (s.rtc !== tentativa) return;
    const stream = (tentativa.stream ??= new MediaStream());
    ...
    if (event.track.kind === 'audio') s.video.muted = false;
    s.video.play().then(
      () => {
        if (s.rtc !== tentativa) return;
        s.rtcConnected = true;
        ...
        s.audio?.stop();
        s.audio = null;
        clearTimeout(tentativa.timer);
        renderGrid();
        renderBar();
      },
      () => {
        // Autoplay com áudio pode ser bloqueado fora do gesto de assistir.
        // Mantemos o vídeo direto e o áudio relay como fallback local.
        s.video.muted = true;
        s.video.play().catch(() => {});
        s.rtcConnected = true;
        s.started = true;
  ...
  const iniciar = async () => {
    try {
      const pc = criarPeer({
        ice: await iceServers(P),
        onIce: enviar,
        onEstado: (state) => {
          if (MORTO.has(state)) fallbackRtc(slot);
        },
        onTrack: receberFaixa,
      });
      if (s.rtc !== tentativa) return pc.close();
      tentativa.pc = pc;
      pc.addTransceiver?.('video', { direction: 'recvonly' });
      pc.addTransceiver?.('audio', { direction: 'recvonly' });
      for (const pending of tentativa.sinais.splice(0)) {
        receberRtc(slot, pending.peerId, pending.signal);
      }
    } catch {
      fallbackRtc(slot);
    }
...
function receberRtc(slot, peerId, signal) {
  const s = streams.get(slot);
  const tentativa = s?.rtc;
  if (!tentativa || !signal) return;
  tentativa.peerId = peerId;

  const pc = tentativa.pc;
  if (!pc) {
    tentativa.sinais.push({ peerId, signal });
    return;
  }
  const concluir = async () => {
    try {
      if (signal.description?.type === 'offer') {
        ...
        for (const candidato of tentativa.candidatos.splice(0)) {
          ws?.send(JSON.stringify({ type: 'rtc-signal', slot, peerId, signal: candidato }));
        }
        for (const candidato of tentativa.candidatosRemotos.splice(0)) {
          await pc.addIceCandidate(candidato);
        }
      } else if (signal.candidate) {
        if (tentativa.remoteReady) await pc.addIceCandidate(signal.candidate);
        else tentativa.candidatosRemotos.push(signal.candidate);
      }
    } catch {
      fallbackRtc(slot);
    }
...
function fallbackRtc(slot) {
  ...
  try {
    ...
    // O navegador já pode ter fechado a conexão.
  }
  if (s.rtc === tentativa) {
    s.rtc = null;
    s.rtcConnected = false;
    s.started = false;
    s.rtcError = 'WebRTC não conseguiu conectar. Verifique a rede e tente novamente.';
    s.video?.srcObject?.getTracks?.().forEach((track) => track.stop());
    s.video = null;
    toast(s.rtcError, true);
    renderGrid();
    renderBar();
  }
}

/** Liga o som relay desta transmissão quando não há áudio via WebRTC. */
function startAudio(slot, config) {
  const s = streams.get(slot);
  if (!s || s.rtcConnected) return;
  s.audio?.stop();
  s.audio = createAudio({ onError: (m) => toast(m, true), volume: volumeEfetivo(s.userId) });
  if (!s.audio.start(config)) s.audio = null;
  renderBar();
}
...
function startStream(slot, config) {
  const s = streams.get(slot);
  if (!s) return;
  const relayStarted = s.player.start(config);
  tentarWebRtc(slot);
...
function connect() {
  ...
  ws.addEventListener('message', (e) => {
    ...
    if (msg.type === 'state') {
      ...
      renderGrid();
    } else if (msg.type === 'rtc-signal' && Number.isInteger(msg.slot)) {
      receberRtc(msg.slot, msg.peerId, msg.signal);
 ...
 */
const AJUSTES_PADRAO = { bitrate: 5000000, fps: 60 };

let ajustes = (() => {
  try {
    const salvo = { ...AJUSTES_PADRAO, ...JSON.parse(read('ajustes') ?? '{}') };
    // Migra a preferência padrão antiga, que limitava gameplay a 30 fps e
    // 2,5 Mb/s. Valores diferentes continuam sendo respeitados.
    if (salvo.bitrate === 2500000 && salvo.fps === 30) return { ...AJUSTES_PADRAO };
    return salvo;
  } catch {
...
async function broadcastFromHere() {
  if (!navigator.mediaDevices?.getDisplayMedia || !suportaWebRTC()) return false;
