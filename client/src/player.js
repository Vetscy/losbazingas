...
export function createPlayer(canvas, { onError, onTamanho } = {}) {
  ...
  function push(buffer) {
    ...
    // VideoDecoder também possui uma fila interna. Se ela cresce, o relay pode
    // estar em dia na rede, mas a imagem continua mostrando o passado.
    if (decoder.decodeQueueSize > 2 && !isKeyframe) return;
  ...
  function draw(frame) {
    // O relay usa TCP e pode entregar vários quadros em uma rajada. Desenhar
    // somente o mais recente por frame do monitor evita reproduzir essa rajada
    // como uma travada, sem acumular atraso.
    if (framePendente) framePendente.close();
    framePendente = frame;
    if (!frameAgendado) frameAgendado = requestAnimationFrame(desenharPendente);
  }

  function desenharPendente() {
    frameAgendado = 0;
    const frame = framePendente;
    framePendente = null;
    if (!frame) return;
