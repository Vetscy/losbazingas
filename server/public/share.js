...
const opcoesSalvas = (() => {
  try {
    return JSON.parse(localStorage.getItem('discord-screen-share-options') ?? '{}');
  } catch {
    return {};
  }
})();
...
const opcoes = {
  bitrate: Number(query.get('q')) || Number(opcoesSalvas.bitrate) || 5_000_000,
  fps: Number(query.get('fps')) || Number(opcoesSalvas.fps) || 60,
  resolution: query.get('res') || opcoesSalvas.resolution || 'source',
};

function salvarOpcoes() {
  try {
    localStorage.setItem('discord-screen-share-options', JSON.stringify(opcoes));
  } catch {
    // O armazenamento pode ser bloqueado; a sessão continua usando a escolha.
  }
}

function atualizarControles() {
  $('quality-bitrate').value = String(opcoes.bitrate);
  $('quality-fps').value = String(opcoes.fps);
  $('quality-resolution').value = opcoes.resolution;
}
...
function aplicarOpcoes(novas) {
  ...
  if (novas.res) opcoes.resolution = String(novas.res);
  salvarOpcoes();
  atualizarControles();
}
...
if (!payload) {
  ...
  atualizarControles();
  $('quality-bitrate').addEventListener('change', (e) => {
    opcoes.bitrate = Number(e.target.value);
    salvarOpcoes();
    aplicarConfig({
      q: String(opcoes.bitrate),
      fps: String(opcoes.fps),
      res: opcoes.resolution,
    });
  });
  $('quality-fps').addEventListener('change', (e) => {
    opcoes.fps = Number(e.target.value);
    salvarOpcoes();
    aplicarConfig({
      q: String(opcoes.bitrate),
      fps: String(opcoes.fps),
  ...
  $('quality-resolution').addEventListener('change', (e) => {
    opcoes.resolution = e.target.value;
    salvarOpcoes();
    aplicarConfig({
      q: String(opcoes.bitrate),
      fps: String(opcoes.fps),
