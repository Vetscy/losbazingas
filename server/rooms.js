...
/**
 * Encaminha apenas sinais entre o transmissor do slot e o espectador que pediu
 * aquele slot. O servidor nunca interpreta SDP nem candidatos ICE.
 */
export function relayRtcSignal(room, sender, slot, peerId, signal) {
  const entry = room.slots.get(slot);
  if (!entry || !entry.streaming || !signal || typeof peerId !== 'string') return false;

  if (sender === entry.ws) {
    const viewer = [...room.viewers].find(
      (candidate) => candidate.__rtcId === peerId && candidate.__watching?.has(slot),
    );
    return viewer ? sendJson(viewer, { type: 'rtc-signal', slot, peerId, signal }) : false;
  }

  if (sender.__rtcId !== peerId || !sender.__watching?.has(slot)) return false;
  return sendJson(entry.ws, { type: 'rtc-signal', slot, peerId, signal });
}
...
export function stopStream(room, entry) {
  ...
  for (const v of room.viewers) {
    if (v.__watching?.has(entry.slot)) {
      sendJson(entry.ws, { type: 'rtc-stop', slot: entry.slot, peerId: v.__rtcId });
    }
    v.__primed?.delete(entry.slot);
    v.__watching?.delete(entry.slot);
  }
...
export function watch(room, ws, slot) {
  ...
  // A negociação WebRTC é opcional. O relay já foi habilitado acima e continua
  // sendo a fonte de recuperação caso o peer não feche a tempo.
  sendJson(entry.ws, {
...
export function unwatch(room, ws, slot) {
  ...
  ws.__primed.delete(slot);
  const entry = room.slots.get(slot);
  if (entry) sendJson(entry.ws, { type: 'rtc-stop', slot, peerId: ws.__rtcId });
  broadcastState(room);
}
...
export function attachViewer(room, ws, info) {
  ...
  ws.__rtcId = crypto.randomBytes(8).toString('base64url');
  room.viewers.add(ws);
  room.emptySince = null;
