...
/**
 * Configuração pública de ICE. O STUN é sempre suficiente para a tentativa
 * experimental; TURN só entra quando foi configurado no ambiente.
 */
app.get('/api/ice', (_req, res) => {
  const iceServers = [{ urls: 'stun:stun.l.google.com:19302' }];
  const urls = String(TURN_URL)
    .split(',')
    .map((url) => url.trim())
    .filter(Boolean);
  if (urls.length) {
...
function handleBroadcaster(ws, room, info, fonte) {
  ...
  ws.on('message', (data, isBinary) => {
    ...
    if (msg.type === 'start') {
      ...
      R.relayRtcSignal(room, ws, msg.slot, msg.peerId, msg.signal);
...
function handleViewer(ws, room, auth) {
  ...
  ws.on('message', (data, isBinary) => {
    ...
    if (msg.type === 'rtc-signal' && Number.isInteger(msg.slot)) {
      R.relayRtcSignal(room, ws, msg.slot, msg.peerId, msg.signal);
      return;
    }
