...
function fitWithin(w, h, resolution = 'source') {
  const limit = resolution === '480' ? 480 : resolution === '720' ? 720 : resolution === '1080' ? 1080 : null;
  const maxW = limit ? Math.min(MAX_W, Math.round(limit * (16 / 9))) : MAX_W;
  const maxH = limit ? Math.min(MAX_H, limit) : MAX_H;
...
export function createBroadcaster({
  ...
  async function start() {
    ...
    const target = fitWithin(s.width ?? 1280, s.height ?? 720, resolution);
  ...
  function encodeFrame(frame) {
    ...
    // Só descartar quando a fila realmente cresceu. Uma fila de um item é
    // normal no encoder e bloquear nesse ponto derruba o FPS.
  ...
  function syncSize(frame) {
    ...
    srcW = sw;
    srcH = sh;
    const target = fitWithin(sw, sh, resolution);
  ...
  function onEncoded(chunk, metadata) {
    ...
    if (ws.bufferedAmount > 750_000 && chunk.type !== 'key') return;
  ...
  function setQuality({ bitrate: nextBitrate, fps: nextFps, resolution: nextResolution } = {}) {
    ...
    stream
      ...
      .catch(() => {});
    if (srcW && srcH && nextResolution) {
      const target = fitWithin(srcW, srcH, resolution);
      if (target.width !== config.width || target.height !== config.height) {
        config = { ...config, ...target };
        encoder.configure(config);
        wantKeyframe = true;
